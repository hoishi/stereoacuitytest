function data = FitData_wblcdf(data)
...
if strcmp(data.type ,'PFC')
    for n =1:length(data.condition)
        ...
        % 心理測定関数の定義
        data.condition(n).fitdata.func  = @(t, p) p(4) + (1 - p(4) - p(3)) .* wblcdf(t,p(1),p(2)); 
    end
...
if strcmp(data.type ,'PFC')
    for n =1:length(data.all)
        ...
        % 心理測定関数の定義
        data.all.fitdata.func  = @(t, p) p(4) + (1 - p(4) - p(3)) .* wblcdf(t,p(1),p(2)); 
    end
...
if strcmp(data.type , 'PCC')
    ...
    for n =1:length(data.all)
        ...
        % 心理測定関数の定義
        data.all(n).fitdata.func  = @(t, p) p(4) + (1 - p(4) - p(3)) .* wblcdf(t,p(1),p(2)); 
    end
...
if strcmp(data.type , 'PCC')
    ...
    for n =1:length(data.left)
        ...
        % 心理測定関数の定義
        data.left.fitdata.func  = @(t, p) p(4) + (1 - p(4) - p(3)) .* wblcdf(t,p(1),p(2)); 
    end
...
if strcmp(data.type , 'PCC')
    ...
    for n =1:length(data.right)
        ...
        % 心理測定関数の定義
        data.right.fitdata.func  = @(t, p) p(4) + (1 - p(4) - p(3)) .* wblcdf(t,p(1),p(2));  
    end
